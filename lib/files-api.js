...
var move = function move(req, res, next, baseDir, baseUrl){
  var resourceUrl = decodeURIComponent(req.url);
  var fullPath = path.join(baseDir, resourceUrl);
  // source (:path), destination (json.id)
  var destPath = req.body.id;
  var fullDestPath = path.join(baseDir, destPath);
  mv(fullPath, path.join(baseDir, destPath), function(err){
    if(err) return next(err);
    fs.stat(fullDestPath, function(err, stats){
      if(err) return next(err);
...
/*
  ...
  PUT    /:path
  moves a dir/file from :path to body.id, returns file/dir json
*/
