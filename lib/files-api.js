...
var copyRemoteFile = function copyRemoteFile(req, res, next, baseDir, baseUrl){
...
var updateTextFile = function updateTextFile(req, res, next, baseDir, baseUrl){
  var resourceUrl = decodeURIComponent(req.url);
  var fullPath = path.join(baseDir, resourceUrl);
...
module.exports = function(options){
  ...
  app.post('/*', express.json(), function(req, res, next){
    ...
    if(req.is('multipart/form-data')){
      ...
      if(req.body.url){
        ...
        copyRemoteFile(req, res, next, options.baseDir, options.baseUrl);
  ...
  app.put('/*', express.json(), function(req, res, next){
    if(req.body.path){
      ...
      updateTextFile(req, res, next, options.baseDir, options.baseUrl);
    }
  });
